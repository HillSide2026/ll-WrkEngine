/**
 * Assistant API client — all requests to the Node.js backend.
...
interface ServerMessage {
    ...
    annotations?: AssistantCitationAnnotation[] | null;
    created_at: string;
}
...
export async function listProjects(): Promise<AssistantProject[]> {
    return apiRequest<AssistantProject[]>("/projects");
}
...
export async function getProject(projectId: string): Promise<AssistantProject> {
    return apiRequest<AssistantProject>(`/projects/${projectId}`);
}
...
): Promise<AssistantFolder> {
    return apiRequest<AssistantFolder>(`/projects/${projectId}/folders`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
            name,
...
export async function renameProjectFolder(
    ...
    return apiRequest<AssistantFolder>(
...
export async function moveDocumentToFolder(
    ...
    return apiRequest<AssistantDocument>(
...
export interface AssistantDocumentVersion {
    id: string;
...
export async function uploadDocumentVersion(
    ...
    file: File,
    displayName?: string,
): Promise<AssistantDocumentVersion> {
    const authHeaders = await getAuthHeader();
    ...
    return response.json() as Promise<AssistantDocumentVersion>;
}
...
export async function renameDocumentVersion(
    ...
    return apiRequest<AssistantDocumentVersion>(
...
export async function uploadProjectDocument(
    ...
    if (!response.ok) throw new Error(await response.text());
    return response.json() as Promise<AssistantDocument>;
}
...
export async function listStandaloneDocuments(): Promise<AssistantDocument[]> {
    return apiRequest<AssistantDocument[]>("/single-documents");
}
...
export async function listChats(): Promise<AssistantChat[]> {
    return apiRequest<AssistantChat[]>("/chat");
}

export async function listProjectChats(projectId: string): Promise<AssistantChat[]> {
    return apiRequest<AssistantChat[]>(`/projects/${projectId}/chats`);
}

export async function getChat(chatId: string): Promise<AssistantChatDetailOut> {
    const raw = await apiRequest<ServerChatDetailOut>(`/chat/${chatId}`);
    const messages: AssistantMessage[] = raw.messages.map((m) => {
        if (m.role === "user") {
            return {
                role: "user",
...
type WorkflowType = AssistantWorkflow["type"];

export async function listWorkflows(
    type: WorkflowType,
): Promise<AssistantWorkflow[]> {
    return apiRequest<AssistantWorkflow[]>(`/workflows?type=${type}`);
}

export async function getWorkflow(workflowId: string): Promise<AssistantWorkflow> {
    return apiRequest<AssistantWorkflow>(`/workflows/${workflowId}`);
}
...
export async function createWorkflow(payload: {
    ...
    return apiRequest<AssistantWorkflow>("/workflows", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
    });
