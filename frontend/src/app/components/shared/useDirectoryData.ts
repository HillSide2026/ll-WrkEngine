...
import { getProject, listProjects, listStandaloneDocuments } from "@/app/lib/api";
import type { AssistantDocument, AssistantProject } from "./types";

const CACHE_TTL_MS = 30_000;

interface DirectoryCache {
    standaloneDocuments: AssistantDocument[];
...
export function useDirectoryData(enabled: boolean) {
    ...
    const [standaloneDocuments, setStandaloneDocuments] = useState<AssistantDocument[]>([]);
