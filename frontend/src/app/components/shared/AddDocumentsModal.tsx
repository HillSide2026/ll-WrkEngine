...
import type { AssistantDocument } from "./types";
...
interface Props {
    open: boolean;
    onClose: () => void;
    onSelect: (documents: AssistantDocument[], projectId?: string) => void;
    breadcrumb: string[];
...
export function AddDocumentsModal({
    ...
    const [extraUploadedDocs, setExtraUploadedDocs] = useState<AssistantDocument[]>([]);
    ...
    async function handleDelete(ids: string[]) {
        ...
        const docsById = new Map<string, AssistantDocument>();
        for (const d of [
            ...standaloneDocuments,
            ...extraUploadedDocs,
            ...projects.flatMap((p) => p.documents ?? []),
