...
import type { AssistantDocument, AssistantMessage } from "../shared/types";

export interface ChatInputHandle {
    addDoc: (doc: AssistantDocument) => void;
}

interface Props {
    onSubmit: (message: AssistantMessage) => void;
    onCancel: () => void;
    isLoading: boolean;
...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput(
    ...
    const [attachedDocs, setAttachedDocs] = useState<AssistantDocument[]>([]);
    ...
    const handleAddDocFromProject = useCallback((doc: AssistantDocument) => {
        setAttachedDocs((prev) => {
            if (prev.some((d) => d.id === doc.id)) return prev;
            return [...prev, doc];
        });
    ...
    const handleAddDocsFromSelector = useCallback(
        (selectedDocs: AssistantDocument[]) => {
            setAttachedDocs((prev) => {
