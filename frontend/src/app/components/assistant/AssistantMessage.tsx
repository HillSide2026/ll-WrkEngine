...
import { AssistantIcon } from "@/components/chat/app-icon";
...
function BulkEditActions({
    ...
    pending: {
        annotation: AssistantEditAnnotation;
        filename: string;
    }[];
    filenameByDocId: Map<string, string>;
    onViewClick?: (ann: AssistantEditAnnotation, filename: string) => void;
    onResolveStart?: (args: {
        editId: string;
        documentId: string;
        verb: "accept" | "reject";
    ...
    return (
        <div className="w-full h-9 flex items-center mb-2">
            <AssistantIcon
                ...
                error={isError}
                active={!isError && !(showDone && doneVisible)}
                size={22}
            />
        </div>
    );
...
function preprocessCitations(
    text: string,
    annotations: AssistantCitationAnnotation[],
    citationsList: AssistantCitationAnnotation[],
): string {
...
function MarkdownContent({
    ...
    text: string;
    citationsList: AssistantCitationAnnotation[];
    onCitationClick?: (c: AssistantCitationAnnotation) => void;
...
interface Props {
    ...
    errorMessage?: string;
    annotations?: AssistantCitationAnnotation[];
    onCitationClick?: (citation: AssistantCitationAnnotation) => void;
    minHeight?: string;
    onWorkflowClick?: (workflowId: string) => void;
    onEditViewClick?: (ann: AssistantEditAnnotation, filename: string) => void;
    /**
...
export function AssistantMessage({
    ...
    const citationsList: AssistantCitationAnnotation[] = [];
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        {!isStreaming &&
                            (() => {
                                ...
                                const statusOf = (ann: AssistantEditAnnotation) =>
